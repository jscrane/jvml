(ns ml.test-ex5
  (:use (clojure test)
        (ml util testutil linear)
        [ml.ex5 :only (init-ex5 polynomial-features validation-curve train-linear-regression)]))

(def approx (approximately 1e-5))

(deftest regularized-linear-regression
  (let [{:keys [X y]} (init-ex5)
        {cost :cost grad :grad} ((reg-linear-cost-function (add-intercept X) y 1) [1 1])]
...
(deftest test-set-error
  (let [{:keys [lambdas X y Xval yval Xtest ytest]} (init-ex5)
    ...
    (is (= 3 lambda-opt))
    (is (approx 3.8599 (:cost ((reg-linear-cost-function Xpoly-test ytest 0) theta))))))